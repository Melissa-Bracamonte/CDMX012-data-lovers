...
function clickSearch() {
  ...
  document.querySelectorAll(".table-sortable th").forEach((headerCell) => {
    headerCell.addEventListener("click", () => {
      ...
      const headerIndex = Array.prototype.indexOf.call(headerCell.parentElement.children,headerCell);//headerIndex obtiene la posición de la columna clickeada
      const currentIsAscending = headerCell.classList.contains("th-sort-asc"); //Determina si los elementos serán ascendenbtes
...
function carrousel(containerCarrusel) {
  containerCarrusel.addEventListener("click", (event) => {
    ...
    let tgt = event.target; //Encontrará el elemento exacto que disparó el evento click (ya sea botón previous o next)
    ...
    if (tgt == previous) {
      if (counter > 0) {
        ...
        image.src = arrayImgCarrousel[arrayImgCarrousel.length - 1]; //para que me devuelva la última imagen e itere de manera infinita
...
document.addEventListener("DOMContentLoaded", () => {
  let containerCarrusel = document.querySelector(".containerCarrusel");
  carrousel(containerCarrusel);
});
